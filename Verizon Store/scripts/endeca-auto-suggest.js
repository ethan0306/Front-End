...
function($j)
{
    ...
    $j.EndecaSearchSuggestor = function(ele, opts)
    {
	 ...
	 this._autoSuggestOn = false;
        ...
        $j(".ui-page .ui-content").append(this._container);
	 ...
	 /**
         ...
         */
	 cancelButton.bind('click', $j.proxy(function(){
		suggestor.hide();
		this._autoSuggestOn = false;
		$j("#" + pageId).removeClass('searchPanelShowing');
		$j("#" + pageId + " .content .ui-input-search input").val('');
		$j("#" + pageId + " .content .search .ui-input-clear").addClass('ui-input-clear-hidden');
		if( pageId == "home" || pageId == "accessDetail" ){
			$j('#' + pageId + ' .search').toggleClass('On');
		}
	 }, this));

	 ele.bind('click', $j.proxy(function(){ 
    ...
    $j.EndecaSearchSuggestor.prototype.generateHtmlContent = function(dimSearchResult) 
    {
	 ...
	 var subContent = null;
        ...
        if(dimSearchResult != null && dimSearchResult.dimensionSearchGroups.length > 0)
        {
		  ...
		  subContent = $j('<ul data-role="listview"></ul>');
